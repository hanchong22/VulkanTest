...
#define STB_IMAGE_IMPLEMENTATION	//开启stb库中的图片工具
#include <stb_image.h>				//读取图片工具	

#define TINYOBJLOADER_IMPLEMENTATION
#include <tiny_obj_loader.h>		//加载3dmax obj文件的第三方库
...
void VulkanBase::createRenderPass()
{
	...
	depthAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;				//每次渲染新的一帧前使用黑色清除帧缓冲
	depthAttachment.storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;			//绘制结束后不需要从深度缓冲中复制深度数据
	depthAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;	//不需要读取之前深度图像
...
void VulkanBase::createGraphicsPipeline() {
	...
	depthStencilStateCreateInfo.depthTestEnable = VK_TRUE;		//开启深度测试
	depthStencilStateCreateInfo.depthWriteEnable = VK_TRUE;		//开启深度写入（不透明）
	depthStencilStateCreateInfo.depthCompareOp = VK_COMPARE_OP_LESS;		//深度测试的比较方式，VK_COMPARE_OP_LESS是指深度值小于深度缓冲的值时才会写入颜色附件
	//指定可选的深度范围测试，暂不使用
	...
	pipelineInfo.pDepthStencilState = &depthStencilStateCreateInfo;	//深度测试
...
void VulkanBase::createCommandBuffer()
{
	...
	for (size_t i = 0; i < commandBuffers.size(); ++i)
	{
		...
		}
		{
			...
			clearValues[0].color = { 0.1f, 0.2f, 0.5f, 1.0f };
...
void VulkanBase::createDepthResources()
{
	...
	//创建深度图
	//深度图为GPU内部写入与读取，无需从CPU传入数据

	createImage(swapChainExtent.width, swapChainExtent.height, depthFormat, 
		VK_IMAGE_TILING_OPTIMAL, 
		VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,		//类型为设备内部内存，CPU无法访问，但GPU访问的性能更好
...
VkFormat VulkanBase::findDepthFormat()
{
	return findSupportedFormat(
		{ 
			...
			VK_FORMAT_D32_SFLOAT_S8_UINT,	//32 位浮点深度值和8位模板值
			VK_FORMAT_D24_UNORM_S8_UINT		//24 位浮点深度值和8位模板值
		},
		//tiling值 ：VK_IMAGE_TILING_LINEAR：纹理以行主序的方式排列，如果要直接访问图片数据，则需以设置为此
		//tiling值 ：K_IMAGE_TILING_OPTIMAL：纹理以一种对访问优化的方式排列,深度图等不需要处理的图片设置为此更好性能
		VK_IMAGE_TILING_OPTIMAL,
		//深度模板用途
...
{
	tinyobj::attrib_t attrib;					//顶点属性(包括顶点的详细数据)	
	std::vector<tinyobj::shape_t> shapes;		//顶点数据（各顶点的索引值）
	std::vector<tinyobj::material_t> materials;		//材质属性（uniform名称与值）
	std::string warn, err;

	if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, MODEL_PATH.c_str())) {
		throw std::runtime_error(warn + err);
	}
	//用map结构，使用顶点做key，可保证顶点不会重复
